...
public class Admin extends Libraire implements GestionUtilisateurs,GestionLibraires{
	...
	/**
	 * M�thode pour r�cup�rer tout les utilisateurs
	 */
	@Override
	public List<Utilisateur> consulterUtilisateurs(){
		...
		try {
			...
			System.out.println("Aucun utilisateur n'a �t� trouv�!");
		}
		return users;
	}
	...
	/**
	 * M�thode d'ajout d'un utilisateur
	 */
	@Override
	public boolean ajouterUtilisateur() {
		...
		try{
			...
			if(resultSet.next()) {
				System.out.println("!!!L'email saisi existe d�ja!!!");
	...
	/**
	 * @param id
	 * @return true si l'utilisateur d'id saisi est supprim�
	 */
	@Override
			...
			for (int i=0; i<users.size(); i++){
				if(users.get(i).getId()==id) {
					...
					if ( preparedStatement.executeUpdate() > 0 ) {
						...
						System.out.println("Erreur! Utilisateur n'a pas �t� supprim�!");
					}
					preparedStatement.close();
					break;
				}
	...
	/**
	 * M�thode pour la modification d'un utilisateur
	 */
	@Override
	...
	/****************M�thodes de gestion des Libraires**********************/
	...
	/**
	 * @param id
	 * @return true si le libraire d'id saisi est supprim�
	 */
	@Override
			...
			for (Libraire libraire : libraires) {
				if(libraire.getId()==id) {
					...
					if ( preparedStatement.executeUpdate() > 0 ) {
						...
						System.out.println("Libraire supprim�");
						isDeleted = true;
					}else {
						System.out.println("Erreur! Libraire n'a pas �t� supprim�!");
					}
					preparedStatement.close();
					break;
				}
	...
	/**
	 * M�thode pour la modification d'un libraire
	 */
